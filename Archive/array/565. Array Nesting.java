...
class Solution {
    public int arrayNesting(int[] nums) {
        ...
        for (int i = 0; i < nums.length; i++) {
            int cnt = 0;
            for (int holder = i; nums[holder] != -1;) {
                // for used elements they just run in the former circle,
                // which is the same length and contributes nothing.
                int t = nums[holder];
                nums[holder] = -1;
                holder = t;
                cnt++;
...
/**
 * Runtime: 6 ms, faster than 87.68% of Java online submissions for Array
 * Nesting.
 * Memory Usage: 76 MB, less than 61.40% of Java online submissions for Array
 * Nesting.
 */